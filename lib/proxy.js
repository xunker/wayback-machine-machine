...
function doWaybackQuery(urlObj, res) {
  ...
  queryURL(checkURL, function(err, response, body) {
    ...
    try {
      ...
      if (bodyObj.archived_snapshots.closest) {
        var ts = bodyObj.archived_snapshots.closest.timestamp;
        var waybackURL = 'http://web.archive.org/web/' + ts + '/';
        return cyberspace(waybackURL, urlObj, res);
      }

      if (config.resolveMissing) {
        log("Resolving missing link by proxying to", bodyObj.url);
        return proxyRequest('http://' + bodyObj.url, res);
      }
...
function proxyRequest(url, res, intercept) {
  ...
  http.get(url, function(response) {
    ...
    if (config.noCache) {
      log('Apply cache-control headers to response to prevent caching');
      newHeaders['cache-control'] = 'no-cache no-store max-age=0';
    }
