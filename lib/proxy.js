...
function proxyRequest(url, res, intercept) {
  http.get(url, function(response) {
    ...
    if (response.headers['set-cookie']) {
      newHeaders['cache-cset-cookieontrol'] = response.headers['set-cookie'];
    }
    if (response.headers['cache-control']) {
      newHeaders['cache-control'] = response.headers['cache-control'];
    }
    if (response.headers['server']) {
      newHeaders['server'] = response.headers['server'];
    }
    ...
    if (response.headers['content-type'].match(/^image\//i)) {
      // delete response.headers['Transfer-Encoding'];
      // newHeaders['transfer-encoding'] = 'none';
      // newHeaders['transfer-encoding'] = undefined;
      // newHeaders['content-length'] = 697;
